...
class _AnalysisScreenState extends ConsumerState<AnalysisScreen> {
  ...
  Widget _buildError() => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text('Analysis Failed', style: GoogleFonts.outfit(
          color: kOnSurface, fontSize: 18, fontWeight: FontWeight.w700)),
  ...
  Widget _buildNoGame() {
    ...
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.analytics_outlined, color: kOutlineVariant, size: 56),
        const SizedBox(height: 16),
        Text('No Game Selected', style: GoogleFonts.outfit(
            color: kOnSurface, fontSize: 20, fontWeight: FontWeight.w700)),
        const SizedBox(height: 8),
  ...
  Widget _buildBoardSection(String evalStr) {
    ...
    return Container(
      decoration: BoxDecoration(color: kSurfaceContLow, borderRadius: BorderRadius.circular(16)),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(_currentMoveIdx < 0 ? 'STARTING POSITION' : 'MOVE ${_currentMoveIdx + 1}',
              style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.w700, color: kOnSurface)),
          const Spacer(),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
            decoration: BoxDecoration(color: kSurfaceContHighest, borderRadius: BorderRadius.circular(99)),
            child: Text('EVAL: $evalStr',
                style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w700,
                    color: kOnSurfaceVariant, letterSpacing: 1)),
          ),
        ]),
        const SizedBox(height: 16),
        AspectRatio(
          ...
          child: Stack(children: [
            Container(
              decoration: BoxDecoration(
                ...
                color: ref.watch(boardThemeProvider).frameColor,
              ...
              child: ClipRRect(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final p = _pieceAt(row, col);
                    final boardTheme = ref.watch(boardThemeProvider);
                    return Container(
                      decoration: BoxDecoration(
                        color: highlighted
                            ? kPrimaryContainer.withValues(alpha: isTo ? 0.45 : 0.22)
                        ...
                        if (p != null)
                          Center(
                            child: Padding(
                              ...
                              child: ChessPieceWidget.fromPiece(piece: p),
                            ),
                          ),
                        if (col == 0) Positioned(top: 2, left: 3,
                            child: Text('${8 - row}', style: GoogleFonts.inter(fontSize: 7, fontWeight: FontWeight.w800,
                                color: isLight ? boardTheme.darkSquare.withValues(alpha: 0.85) : boardTheme.lightSquare.withValues(alpha: 0.85)))),
                        if (row == 7) Positioned(bottom: 1, right: 3,
                            child: Text(files[col], style: GoogleFonts.inter(fontSize: 7, fontWeight: FontWeight.w800,
                                color: isLight ? boardTheme.darkSquare.withValues(alpha: 0.85) : boardTheme.lightSquare.withValues(alpha: 0.85)))),
                      ]),
                    );
                  },
  ...
  Widget _buildNavControls() {
    return Row(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(color: kSurfaceContLow, borderRadius: BorderRadius.circular(10)),
        child: Text(
          _currentMoveIdx < 0
              ? 'Start'
              : '${_currentMoveIdx + 1} / ${_report!.moves.length}',
          style: GoogleFonts.inter(color: kOnSurface, fontSize: 13, fontWeight: FontWeight.w600),
        ),
      ),
      const SizedBox(width: 8),
  ...
  Widget _navBtn(IconData icon, VoidCallback onTap) {
    return Material(
      color: kSurfaceContLow, borderRadius: BorderRadius.circular(10),
      child: InkWell(
  ...
  Widget _buildEvalGraph() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('EVALUATION GRAPH', style: GoogleFonts.outfit(
            fontSize: 10, fontWeight: FontWeight.w700, color: kOnSurfaceVariant, letterSpacing: 2)),
        const SizedBox(height: 20),
        SizedBox(
          height: 160,
          child: LineChart(LineChartData(
            ...
            gridData: FlGridData(
              ...
              getDrawingHorizontalLine: (v) => FlLine(
                  color: v == 0 ? kPrimary.withValues(alpha: 0.3) : kOutlineVariant.withValues(alpha: 0.15),
              ...
              touchTooltipData: LineTouchTooltipData(
                getTooltipItems: (spots) => spots.map((s) =>
                    LineTooltipItem('${s.y >= 0 ? "+" : ""}${s.y.toStringAsFixed(1)}',
                        GoogleFonts.inter(color: kPrimary, fontWeight: FontWeight.w700, fontSize: 11))
                ).toList(),
              ),
            ),
            lineBarsData: [
              LineChartBarData(
                ...
                color: kPrimary, barWidth: 2, isStrokeCapRound: true,
                dotData: FlDotData(show: false),
                belowBarData: BarAreaData(
                  show: true,
                  gradient: LinearGradient(
                    begin: Alignment.topCenter, end: Alignment.bottomCenter,
                    colors: [kPrimary.withValues(alpha: 0.15), Colors.transparent],
                  ),
                ),
              ),
            ],
            extraLinesData: ExtraLinesData(horizontalLines: [
              HorizontalLine(y: 0, color: kOutlineVariant.withValues(alpha: 0.3), strokeWidth: 1),
            ]),
          )),
        ),
        const SizedBox(height: 12),
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Move 1', style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w700, color: kOutlineVariant)),
          Text('Move ${_report!.moves.length}', style: GoogleFonts.inter(fontSize: 9, fontWeight: FontWeight.w700, color: kOutlineVariant)),
        ]),
      ]),
    );
  }
  ...
  Widget _buildMoveRow(int moveNum, List<AnalyzedMove> pair) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: (whiteSelected || blackSelected) ? kPrimary.withValues(alpha: 0.05) : Colors.transparent,
        borderRadius: BorderRadius.circular(8),
      ),
      child: Row(children: [
        SizedBox(width: 28, child: Text('$moveNum.',
            style: GoogleFonts.inter(fontSize: 11, color: kOutlineVariant,
                fontFeatures: [const FontFeature.tabularFigures()]))),
        Expanded(child: GestureDetector(
          onTap: () => _goToMove(whiteIdx),
          child: Row(children: [
            Text(white.san, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700,
                color: whiteSelected ? kPrimary : kOnSurface)),
          ...
          Expanded(child: GestureDetector(
            ...
            child: Row(children: [
              Text(black.san, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700,
                  color: blackSelected ? kPrimary
                      : (black.tag == 'mistake' || black.tag == 'blunder') ? kError : kOnSurface)),
              const SizedBox(width: 6),
              _tagChip(black.tag),
            ]),
          ))
  ...
  Widget _tagChip(String tag) {
    ...
    String label = tag;
    switch (tag) {
      case 'brilliant': bg = kPrimary.withValues(alpha: 0.2); fg = kPrimary; label = '!!'; break;
      case 'great': bg = kPrimary.withValues(alpha: 0.15); fg = kPrimary; label = '!'; break;
      case 'best': bg = kSurfaceContHighest; fg = kOutlineVariant; break;
      ...
      case 'mistake': bg = kError.withValues(alpha: 0.2); fg = kError; label = '?'; break;
      case 'blunder': bg = kError.withValues(alpha: 0.3); fg = kError; label = '??'; break;
  ...
  Widget _buildAiInsight() {
    ...
    return Container(
      decoration: BoxDecoration(
        color: kSurfaceContHighest, borderRadius: BorderRadius.circular(16),
        ...
        Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(width: 40, height: 40,
              decoration: BoxDecoration(color: kPrimary.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.psychology, color: kPrimary, size: 22)),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('ENGINE INSIGHT', style: GoogleFonts.outfit(
                  fontSize: 10, fontWeight: FontWeight.w700, color: kPrimary, letterSpacing: 2)),
              const SizedBox(height: 8),
              RichText(text: TextSpan(
                ...
                children: [
                  ...
                  TextSpan(text: insight.played,
                      style: GoogleFonts.inter(color: kError, fontWeight: FontWeight.w700)),
              ...
              GestureDetector(
                ...
                child: Row(children: [
                  Text('GO TO MOVE', style: GoogleFonts.inter(
                      fontSize: 9, fontWeight: FontWeight.w700, color: kPrimary, letterSpacing: 2)),
                  const SizedBox(width: 4),
                  const Icon(Icons.arrow_forward, color: kPrimary, size: 14),
                ]),
              ),
            ])),
          ]),
